      ...
      ? [
          {
            id: itemId,
            status: "returned",
            actual_returned_at: actualReturnedAtIso,
          },
        ]
...
function finalizeReturn(backendSuccess, backendResponse, requestId, itemId) {
  ...
  if (itemIndex !== -1) {
    ...
    try {
      ...
      console.warn(
        "finalizeReturn: failed to archive returned item from backend payload",
        e,
      );
    }
  }
...
window.openReturnDateModal = openReturnDateModal;
window.closeReturnDateModal = closeReturnDateModal;
